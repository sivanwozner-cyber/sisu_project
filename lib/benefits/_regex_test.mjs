// Throwaway verification script — tests blockSplit/field regex against real fetched markdown sample.
// Run: node lib/benefits/_regex_test.mjs
...
![Food Appeal](/_next/image?url=https%3A%2F%2Fexample.com%2Flogo3.png)

![סט 5 חלקים](/_next/image?url=https%3A%2F%2Fexample.com%2Fproduct3.png)
...
![share](/_next/image?url=share-icon3.png)
`;
...
// split into blocks: prepend a sentinel split, then filter
const parts = sampleMarkdown.split(blockSplit).filter(b => /###\s+.+ב-[\d.,]+\s*₪\s*במקום\s*[\d.,]+\s*₪/.test(b));
...
parts.forEach((block, i) => {
  ...
  const discountAmount = (salePrice != null && origPrice != null) ? +(origPrice - salePrice).toFixed(2) : null;
