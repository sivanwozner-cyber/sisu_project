...
## Open Questions
- Exact price-portal URLs + credentials per chain (to be discovered by `price-feed-ingestor` agent).
- `scrape_targets` for benefit sources not yet discovered (`scrape-target-builder` agent).
- Representative branch vs aggregation per chain for MVP prices.
- Build plan awaiting user approval before any code/agents/skills are created.
